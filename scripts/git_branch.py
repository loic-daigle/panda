...
def get_base_version(project_dir):
    ...
    if not config.has_option('panda', 'version'):
        warn('No [panda] version in platformio.ini; base version will be "0.0.0"')
        return '0.0.0'
    return config.get('panda', 'version')
...
def inject_version(env):
    ...
    print(f'panda build version: {version_string}')
