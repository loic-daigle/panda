...
// Major world cities with latitude / longitude. Format: {"City", lat, lon}.
...
constexpr int CITY_COUNT = sizeof(CITIES) / sizeof(CITIES[0]);
...
void saveConfig(int cityIndex, const std::string& cityNameIn) {
  Storage.ensureDirectoryExists("/biscuit");
  Storage.ensureDirectoryExists("/biscuit/weather");
  JsonDocument doc;
  doc["city_index"] = cityIndex;
  doc["city_name"] = cityNameIn;
  String output;
  serializeJson(doc, output);
  Storage.writeFile("/biscuit/weather/config.json", output);
}

bool loadConfig(int& cityIndex, std::string& cityNameOut) {
  String input = Storage.readFile("/biscuit/weather/config.json");
  if (input.length() == 0) return false;
  JsonDocument doc;
  DeserializationError err = deserializeJson(doc, input);
  if (err) return false;
  cityIndex = doc["city_index"] | 0;
  cityNameOut = doc["city_name"] | "";
  return true;
}
...
void saveCache(const std::string& cityNameIn, double tempIn, double windspeedIn, int weathercodeIn,
               const std::string& timeStrIn) {
  Storage.ensureDirectoryExists("/biscuit");
  Storage.ensureDirectoryExists("/biscuit/weather");
  JsonDocument doc;
  doc["temp"] = tempIn;
  doc["windspeed"] = windspeedIn;
  doc["weathercode"] = weathercodeIn;
  doc["time"] = timeStrIn;
  String output;
  serializeJson(doc, output);
  const std::string filepath = "/biscuit/weather/" + getSafeCityFilename(cityNameIn) + ".txt";
  Storage.writeFile(filepath.c_str(), output);
}

bool loadCache(const std::string& cityNameIn, double& tempOut, double& windspeedOut, int& weathercodeOut,
  ...
  DeserializationError err = deserializeJson(doc, input);
  if (err) return false;
  tempOut = doc["temp"] | 0.0;
  windspeedOut = doc["windspeed"] | 0.0;
  weathercodeOut = doc["weathercode"] | 0;
  timeStrOut = doc["time"] | "";
  return true;
}
...
bool fetchWeather(double lat, double lon, double& tempOut, double& windspeedOut, int& weathercodeOut,
  ...
  tempOut = cw["temperature"] | 0.0;
  windspeedOut = cw["windspeed"] | 0.0;
  weathercodeOut = cw["weathercode"] | 0;
  timeStrOut = cw["time"] | "";
  return true;
}

// Hand-drawn (vector) weather glyph — no bitmap asset needed. Uses only
// GfxRenderer primitives so it costs no flash beyond this code.
...
void weatherFetchTaskFunc(void* param) {
  auto* activity = static_cast<WeatherActivity*>(param);
...
void WeatherActivity::runBackgroundFetch() {
  // Guards against the blocking WiFi connect / HTTP call below hanging
  // forever (e.g. captive portal, bad credentials): forces WiFi off after
  // the timeout so the calls below unblock instead of stalling the task.
  DownloadWatchdog::start(20000);

  if (WifiConnectHelper::connectToDefaultWifi() && !DownloadWatchdog::gotTimeout) {
    // Small stabilization delay for DNS resolution right after association.
    delay(500);

    const double lat = CITIES[selectedCityIndex].lat;
    const double lon = CITIES[selectedCityIndex].lon;

    int fetchRetries = 3;
    backgroundFetchSuccess = false;
    while (fetchRetries > 0 && !DownloadWatchdog::gotTimeout) {
      if (fetchWeather(lat, lon, fetchedTemp, fetchedWindspeed, fetchedWeatherCode, fetchedTimeStr)) {
        backgroundFetchSuccess = true;
        break;
      }
      fetchRetries--;
      if (fetchRetries > 0) delay(1000);
    }
  } else {
    backgroundFetchSuccess = false;
  }
...
void WeatherActivity::onEnter() {
  ...
  if (loadConfig(selectedCityIndex, cityName)) {
    if (loadCache(cityName, temp, windspeed, weatherCode, timeStr)) {
      ...
      // Only kick off a background refresh if WiFi is already up — don't
      // force the radio on just to show a screen that already has data.
...
void WeatherActivity::performFetch() {
  ...
  RADIO.ensureWifi();
  backgroundFetchSuccess = false;
  xTaskCreate(weatherFetchTaskFunc, "weather_fetch", 8192, this, 5, reinterpret_cast<TaskHandle_t*>(&fetchTaskHandle));
}
...
void WeatherActivity::loop() {
  if (pendingUpdateWeather) {
    ...
    // A fetch attempt always resolves the Loading screen, whether it
    // succeeded, fell back to previously-cached data, or failed outright.
  ...
  if (state == WeatherState::SelectCity) {
    buttonNavigator.onNext([this] {
      selectedCityIndex = ButtonNavigator::nextIndex(selectedCityIndex, CITY_COUNT);
      requestUpdate();
    });
    buttonNavigator.onPrevious([this] {
      selectedCityIndex = ButtonNavigator::previousIndex(selectedCityIndex, CITY_COUNT);
      requestUpdate();
    });
...
void WeatherActivity::render(RenderLock&&) {
  ...
  if (state == WeatherState::SelectCity) {
    ...
    GUI.drawList(
        renderer, Rect{0, contentTop, pageWidth, contentHeight}, CITY_COUNT, selectedCityIndex,
        [](int index) -> std::string { return CITIES[index].name; }, nullptr,
        [](int) -> UIIcon { return UIIcon::Library; });
    ...
    if (weatherLoaded) {
      ...
      if (fetchTaskHandle != nullptr) {
        ...
        const size_t tPos = formattedTime.find('T');
      ...
      const int textY = cardY + cardH / 2 - renderer.getLineHeight(UI_10_FONT_ID) / 2;
      renderer.drawCenteredText(UI_10_FONT_ID, textY, errorMessage.c_str(), true, EpdFontFamily::BOLD);
    }

    const auto labels = mappedInput.mapLabels("Back", "Refresh", "City", "");
    GUI.drawButtonHints(renderer, labels.btn1, labels.btn2, labels.btn3, labels.btn4);
  }
